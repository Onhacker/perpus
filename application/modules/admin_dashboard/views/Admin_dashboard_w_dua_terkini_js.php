   ...
   Highcharts.chart('tren_terkini_w_dua', {
    ...
    categories: [<?php  
            foreach ($resi->result() as $row) : 
                        ...
                        echo "' ".$this->om->bentuk_admin($row->id_pkma,"l")." ".(($row->nama_pkm))."',";
    ...
    series: [
    {
        ...
        data: [<?php foreach($resi->result() as $row) : 
            $dcf = $this->db->query("select * from w_dua where id_pkm = ".$row->id_pkma."  and  tahun  = '".$wakti->tahun."' and minggu_ke = '".$ming."'  GROUP by id_pkm");
