<script type="text/javascript">
    ...
    Highcharts.chart('tampil_stat', {
        ...
        title: {
             text: 'Pemantauan Wilayah Setempat (PWS) Dinas Kesehatan <?php echo ucwords(strtolower($this->om->web_me()->kabupaten)).". ".$vaksin ?>',
        },
        ...
        // },
          ...
          [<?php  
            foreach ($desa2->result() as $v) :
                $this->db->where("id_pkm", $v->id_pkm);
                $this->db->select("sum(bayi_si_l+bayi_si_p) as jumb")->from("im_tahun_vaksin_isi");
                $by = $this->db->get()->row();
                // echo $this->db->last_query();
                $byj = $by->jumb;
                $bysi = 100/$byj;
                if ($jenis_vaksin == "9999") {
                    $this->db->where(array("id_pkm" => $v->id_pkm)); 
...
<script type="text/javascript">
    Highcharts.chart('tampil_bulan', {
        ...
        title: {
            ...
            text: '% <?php echo $waktu_awal ?><br>Dinas Kesehatan Kabupaten <?php echo ucwords(strtolower($this->om->web_me()->kabupaten))." ".$vaksin ?>',
...
<script type="text/javascript">
    ...
    Highcharts.chart('tampil_trend', {
        ...
        title: {
            ...
            ?>
            text: 'Trend <?php echo $waktu_awal ?><br>Dinas Kesehatan Kabupaten <?php echo ucwords(strtolower($this->om->web_me()->kabupaten))." ".$vaksin ?>',
        ...
        <?php foreach ($desa->result() as $v) :?>
            {
                name : '<?php echo $v->nama_pkm ?>',
                data : [
                <?php $this->db->where("id_pkm", $v->id_pkm);
                $by = $this->db->get("im_tahun_vaksin_isi")->row();
                $byj = $by->bayi_si_l + $by->bayi_si_p;
                $bysi = 100/$byj;
                if ($jenis_vaksin == "9999") {
