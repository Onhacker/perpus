...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('#btn-print').hide(); 
        $('.spinner-grow').hide(); 
        $('#id_pkm,#jenis_vaksin_form,#tahun,#bulan_awal').select2();
        <?php if ($this->session->userdata("admin_level") != "admin") {?>
            load_pws();
    ...
    function reset(){
        ...
        $('#jenis_vaksin_form,#id_pkm').val('x').trigger('change');
        <?php if ($this->session->userdata("admin_level") != "admin") {?>
            load_pws();
    ...
    function load_pws() {
        ...
        <?php if ($this->session->userdata("admin_level") == "admin") {?> 
            id_pkm = $("#id_pkm").val();
            if (tahun == "x"  || bulan_awal == "x" || id_pkm=="x" || jenis_vaksin_form == "x") {
                swal({   
                   title: "Peringatan",   
                   type: "warning", 
                   text: "Silahkan Pilih PKM, Tahun, Bulan dan Jenis Vaksin",
                ...
                $.ajax({
                    <?php if ($this->session->userdata("admin_level") == "admin") {?> 
                       url :'<?php echo site_url(strtolower($controller)."/load_pws"); ?>/'+jenis_vaksin_form+'/'+tahun+'/'+bulan_awal+'/'+id_pkm,
                   <?php } else {?>
