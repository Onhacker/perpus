...
class Jurnal extends Onhacker_Controller {
	function __construct(){
		parent::__construct();
		$this->timezone();
		$this->load->model("M_jurnal", "dm");
	}

	function index(){
		$data["controller"] = get_class($this);	
		$data['title'] = "JURNAL ".$this->fm->web_me()->nama_website;
    ...
    function get_data(){  
        ...
        foreach ($list as $res) {
            ...
            <br><br>
            <button onclick="view('.$res->id_jurnal.') " type="button" class="btn btn-blue btn-xs waves-effect waves-light"><i class=" mdi mdi-file-pdf-box mr-1"></i> Lihat Jurnal</button>
            </p>
           ' ;
           // $row["file"] = 
            $data[] = $row;
