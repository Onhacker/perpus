...
class M_admin_imunisasi_luar extends CI_Model {
	var $table = 'imunisasi_luar';
	var $column_order = array('','','id_imunisasi','jenis_vaksin','nama','jk','tgl_lahir','tgl_suntik','id_pkm');
	...
	private function get_data_query(){
        ...
        if($this->input->post('id_imunisasi')) {
            $this->db->like('id_imunisasi', $this->input->post('id_imunisasi'));
        }
 ...
 //        $this->db->order_by("id_agama", "ASC");
	...
	function arr_detail_pekerjaan(){
        $this->db->order_by("id_pekerjaan", "ASC");
        // $this->db->not_like("id_pekerjaan","89");
        // $this->db->not_like("id_pekerjaan","1");
		$res = $this->db->get("im_pekerjaan");
		// $arr[""]  = "== Pilih Detail Pekerjaan ==";
		foreach($res->result() as $row) :
	...
	function arr_pek(){
		$arr[""]  = "== Pilih Pekerjaan ==";
		$arr["1"]  = "Petugas Medis dan Non Medis di Fasilitas Pelayanan Kesehatan";
		$arr["2"]  = "Petugas pelayanan publik yang berhadapan langsung dengan masyarakat";
		$arr["3"] = "Administrator Pemerintahan";
		$arr["4"] = "Lainnya";
		$arr["5"] = "Tidak Bekerja";
	...
	function arr_tahun(){
		...
		$res = $this->db->get("imunisasi_luar");
		$arr[""]  = "== Semua Tahun == ";
		foreach($res->result() as $row) :
