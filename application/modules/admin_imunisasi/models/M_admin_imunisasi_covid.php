...
class M_admin_imunisasi_covid extends CI_Model {
	var $table = 'imunisasi_covid';
	var $column_order = array('','','no_kia','nama','jk','tgl_lahir','alamat','no_hp','id_imunisasi_covid','id_pkm');
	...
	private function get_data_query(){
        ...
        if($this->input->post('no_kia')) {
            $this->db->like('no_kia', $this->input->post('no_kia'));
        }
 ...
 //        $this->db->order_by("id_agama", "ASC");
	...
	function arr_tahun(){
		...
		$res = $this->db->get("imunisasi_covid");
		$arr[""]  = "== Semua Tahun == ";
		foreach($res->result() as $row) :
