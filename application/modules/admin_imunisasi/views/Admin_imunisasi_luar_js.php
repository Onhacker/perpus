<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('#tahun_cari,#id_pkm,#bulan_cari,#jenis_vaksin_form').select2();
        $('#jenis_vaksin_form').select2({
        ...
        table = $('#datable_1').DataTable({
            ...
            bFilter: false,
            ajax: {
                "url": "<?php echo site_url(strtolower($controller)."/get_data_luar")?>", 
                "type": "POST",
                "data": function ( data ) {
                data.nama = $('#nama_cari').val();
                data.id_imunisasi = $('#no_kia_cari').val();
    ...
    function reset_select(){
        $('#id_pkm,#tahun_cari,#bulan_cari,#jenis_vaksin_form').val('').trigger('change');
    }
    ...
    function edit() {
        ...
        if(list_id.length == 1) { 
            save_method = 'update';
            $.ajax({
                url : "<?php echo site_url(strtolower($controller).'/edit_luar')?>/" + list_id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    ...
                    $("#jk").val(data.jk);
                    $('#jenis_vaksin_form').val(data.jenis_vaksin).trigger('change');
    ...
    function simpan(){
        var url;
        if(save_method == 'add') {
            url = "<?php echo site_url(strtolower($controller).'/add_luar')?>";
        } else {
            url = "<?php echo site_url(strtolower($controller).'/update_luar')?>";
        }   
        ...
        if(list_id.length > 0) { 
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type: "POST",
                        url : "<?php echo site_url(strtolower($controller).'/hapus_data_luar')?>/",
                        data: {id:list_id},
                        cache : false,
                        dataType: "json",
                        success: function(result) {
    ...
    function cetak() {
        ...
        if(list_id.length == 1) { 
            window.open("<?php echo site_url(strtolower($controller)."/pdf_luar/") ?>"+list_id)
        } else if (list_id.length >= 2) {
