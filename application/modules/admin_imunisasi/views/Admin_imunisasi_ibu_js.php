<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        table = $('#datable_1').DataTable({
            ...
            bFilter: false,
            ajax: {
                "url": "<?php echo site_url(strtolower($controller)."/get_data_ibu")?>", 
                "type": "POST",
                "data": function ( data ) {
                ...
                data.jenis_vaksin = $('#jenis_vaksin_formx').val();
                data.jk = $('#jk_cari').val();
                }
            },
    ...
    function edit() {
        ...
        if(list_id.length == 1) { 
            save_method = 'update';
            $.ajax({
                url : "<?php echo site_url(strtolower($controller).'/edit_ibu')?>/" + list_id,
                type: "GET",
                dataType: "JSON",
                success: function(data) {
                    Swal.close();
                    $('#id_ibu').val(data.id_ibu);
                    $('#id_imunisasi_ibu').val(data.id_imunisasi_ibu);
                    $("#detail").show();
                    // $("#isi_detail").show();
                    $("#nama").val(data.nama);
                    ...
                    $('#jenis_vaksinx').val(data.jenis_vaksin);
    ...
    function simpan(){
        var url;
        if(save_method == 'add') {
            url = "<?php echo site_url(strtolower($controller).'/add_ibu')?>";
        } else {
            url = "<?php echo site_url(strtolower($controller).'/update_ibu')?>";
        }   
        ...
        if(list_id.length > 0) { 
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        type: "POST",
                        url : "<?php echo site_url(strtolower($controller).'/hapus_data_ibu')?>/",
                        data: {id:list_id},
                        cache : false,
                        dataType: "json",
                        success: function(result) {
    ...
    function cetak() {
        ...
        if(list_id.length == 1) { 
            window.open("<?php echo site_url(strtolower($controller)."/pdf_ibu/") ?>"+list_id)
        } else if (list_id.length >= 2) {
...
function show_dialog(target) {
     ...
     $('.modal-title-cari').text('Cari Ibu'); // Set Title to Bootstrap modal title
} 
...
function search_cari(){
    ...
    v_id_ibu = $("#search_nama_table").val();
    v_target = $("#target").val();
    console.log(" nama " + v_id_ibu);
    $.ajax({
        url : '<?php echo site_url(strtolower($controller)."/cari_ibu"); ?>',
        type : "post",
        data : {nama  : v_id_ibu, target : v_target },
        success : function(html) {
            $("#ketikan").text(v_id_ibu);
            $("#kotak_pencarian").html(html);
        }
    });
}

function pilih(v_id_ibu, v_id_ibu, target) {
    console.log(v_id_ibu + v_id_ibu + target) ;
    $(target).val(v_id_ibu);
        $("#detail").fadeIn();
        $("#nama" ).focus();
...
$("#nama").focus(function(){
        ...
        $.ajax({
            url : '<?php echo site_url(strtolower($controller)."/get_data_parent_ibu"); ?>',
            data : { id_ibu : $("#id_ibu").val() }, 
            type : 'post', 
            dataType : 'json',
