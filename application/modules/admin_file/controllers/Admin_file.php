...
class Admin_file extends Admin_Controller {
	function __construct(){
		...
		$this->load->model("M_admin_file", "dm");
	}
	...
	function get_data(){   
        ...
        foreach ($list as $res) {
          $no++;
          $row = array();
          $row["id"] = $res->id_jurnal;
          if (empty($res->file)) {
            $row["file"] = "<span class='text-danger'>
            File tidak ada
            </span>";
          } else {
            $row["file"] = "<a href='#' onclick='view(".$res->id_jurnal.") '>Lihat</a>";
            // $row["file"] = $res->file;
          }
          $row["judul"] = $res->judul;
          ...
          $row['cek'] = '<div class="checkbox checkbox-primary checkbox-single"> <input type="checkbox" class="data-check" value="'.$res->id_jurnal.'"><label></label></div>';
    ...
    if($this->form_validation->run() == TRUE ) { 
        ...
        $config['allowed_types'] = 'pdf';
            ...
            if (! $this->upload->do_upload('file')) {
                ...
                $res  = $this->db->insert("jurnal",$data);   
            }
