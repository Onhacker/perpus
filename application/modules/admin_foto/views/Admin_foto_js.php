<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        table = $('#datable_1').DataTable({
            ...
            columns: [
                ...
                {"data": "gbr_album","orderable": false},
                {"data": "judul"}, 
                {"data": "jum","orderable": false},
                {"data": "tanggal"}, 
                {"data": "judul_seo"},
    ...
    function add(){ 
        ...
        $("#tgl_posting").datepicker("setDate",d);
        $("#jam").val(n+':'+m+':'+s );
        $('#previewImage').attr('src', '<?php echo base_url("upload/gambar/upload-here.png") ?>');
        $("#hapus_gambar").hide();
        $("#text_upload").hide();
        ...
        if(list_id.length == 1) { 
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#jdl_album').val(data.jdl_album);
                    $('#jam').val(data.jam);
                    $('#tgl_posting').val(data.tgl_posting);
                    ...
                    if(data.gbr_album == ""){
                        ...
                        $('#previewImage').attr('src', '<?php echo base_url("upload/gambar/") ?>' + data.gbr_album);
                        $("#hapus_gambar").show();  
                        $("#text_upload").show();
                    }
                    $('#summernote').summernote('code', data.keterangan);
                    $('#full-width-modal').modal('show'); 
                    $('.mymodal-title').html('Edit Data <code>'+ data.jdl_album+'</code>'); 
                },
                error: function (jqXHR, textStatus, errorThrown) {
    ...
    function upload_foto() {
        ...
        if(list_id.length == 1) { 
            window.location.href = "<?php echo site_url(strtolower($controller)."/load_drop/") ?>"+list_id;
        } else if (list_id.length >= 2) {
            Swal.fire("Info","Tidak dapat mengupload "+list_id.length+" album sekaligus, Pilih satu data saja", "warning");
        } else {
            Swal.fire("Info","Pilih Satu album", "warning");
        }
    }

    function load_dropx(list_id){
         window.location.href = "<?php echo site_url(strtolower($controller)."/load_drop/") ?>"+list_id;
    }
    ...
    function hapus_data() {
        ...
        if(list_id.length > 0) { 
            Swal.fire({
                ...
                text: "Gambar dalam album akan terhapus dan tidak dapat mengembalikan data terhapus",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#d33",
                cancelButtonColor: "#3085d6",
